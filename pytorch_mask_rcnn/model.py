...
def printMemoryUsage(tag='Memory Usage:'):
    bytes_used = defaultdict(int)
    for obj in gc.get_objects():
        if torch.is_tensor(obj):
            bytes_used[str(obj.device)] += obj.element_size() * obj.numel()
    print(' '.join(
        [tag] + [
            "%s=%0.2fM" % (k, v / 1e6)
            for k, v in bytes_used.items()]))
...
class FPN(nn.Module):
    ...
    def forward(self, x):
        x = self.C1(x)
        printMemoryUsage("After C1:")
        x = self.C2(x)
        printMemoryUsage("After C2:")
        c2_out = x
        x = self.C3(x)
        printMemoryUsage("After C3:")
        c3_out = x
        x = self.C4(x)
        printMemoryUsage("After C4:")
        c4_out = x
        x = self.C5(x)
        printMemoryUsage("After C5:")
        p5_out = self.P5_conv1(x)
        printMemoryUsage("After P5:")
        p4_out = self.P4_conv1(c4_out) + F.upsample(p5_out, scale_factor=2)
        printMemoryUsage("After P4:")
        p3_out = self.P3_conv1(c3_out) + F.upsample(p4_out, scale_factor=2)
        printMemoryUsage("After P3:")
        p2_out = self.P2_conv1(c2_out) + F.upsample(p3_out, scale_factor=2)
        printMemoryUsage("After P2:")

        p5_out = self.P5_conv2(p5_out)
        p4_out = self.P4_conv2(p4_out)
        p3_out = self.P3_conv2(p3_out)
        p2_out = self.P2_conv2(p2_out)
        printMemoryUsage("After FPN conv out:")

        # P6 is used for the 5th anchor scale in RPN. Generated by
        # subsampling from P5 with stride of 2.
        p6_out = self.P6(p5_out)
        printMemoryUsage("After P6:")
...
class Classifier(nn.Module):
    ...
    def forward(self, x, rois):
        ...
        x = self.bn2(x)
        x = self.relu(x)
        printMemoryUsage("After Classifier conv:")
        ...
        printMemoryUsage("After Classifier heads:")
...
class Mask(nn.Module):
    ...
    def forward(self, x, rois):
        printMemoryUsage("Before mask RoiAlign:")
        x = pyramid_roi_align([rois] + x, self.pool_size, self.image_shape)
        printMemoryUsage("After mask RoiAlign:")
        ...
        x = self.conv5(x)
        x = self.sigmoid(x)
        printMemoryUsage("After mask head:")
...
class MaskRCNN(nn.Module):
    ...
    def predict(self, input, mode):
        ...
        for p in rpn_feature_maps:
            ...
            printMemoryUsage("After RPN iteration:")
        ...
        printMemoryUsage("Before NMS:")
        rpn_rois = proposal_layer([rpn_class, rpn_bbox],
                                 proposal_count=proposal_count,
                                 nms_threshold=self.config.RPN_NMS_THRESHOLD,
                                 anchors=self.anchors,
                                 config=self.config)
        printMemoryUsage("After NMS:")
        ...
        if mode == 'inference':
            ...
            printMemoryUsage("Before detection layer:")
            detections = detection_layer(self.config, rpn_rois, mrcnn_class, mrcnn_bbox, image_metas)
            printMemoryUsage("After detection layer:")
